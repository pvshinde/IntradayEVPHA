...
for d=1:9
    Td=[5,6,7,8,9,10,11,12,13]
        @constraint(model, [t=Td[d]:13], pA[d,t]==0)
        @constraint(model, [t=Td[d]:13], pB[d,t]==0)
end
...
scenarios = [PriceScenario([price_scen_to[1][i:i,:]; price_scen_to[2][i:i,:];price_scen_to[3][i:i,:]; price_scen_to[4][i:i,:];price_scen_to[5][i:i,:];
 ...
 lambda_Ip_14[i:i,:], lambda_Im_14[i:i,:]) for i in 1:4096]
...
# price_scenarios_from_9[i:i,9:12]], [price_reg_to_1[i:i,1:1]; price_reg_to_2[i:i,1:1]; price_reg_to_3[i:i,1:1]], [price_reg_from_1[i:i,1:1]; price_reg_from_2[i:i,1:1]; price_reg_from_3[i:i,1:1]]) for i in 1:8]
    ...
    stageid_to_scenpart = [
        OrderedSet([BitSet(4096)]), # Stage 1
        OrderedSet([BitSet(1+2048*i:2048*(1+i)) for i in 0:1]), # Stage 2
        OrderedSet([BitSet(1+1024*i:1024*(1+i)) for i in 0:3]), #stage 3
        OrderedSet([BitSet(1+512*i:512*(1+i)) for i in 0:7]), # stage 4
        OrderedSet([BitSet(1+256*i:256*(1+i)) for i in 0:15]), # stage 5
        OrderedSet([BitSet(1+128*i:128*(1+i)) for i in 0:31]), # stage 6
        OrderedSet([BitSet(1+64*i:64*(1+i)) for i in 0:63]), # stage 7
        OrderedSet([BitSet(1+32*i:32*(1+i)) for i in 0:127]), # stage 8
        OrderedSet([BitSet(1+16*i:16*(1+i)) for i in 0:255]), # stage 9
        OrderedSet([BitSet(1+8*i:8*(1+i)) for i in 0:511]), # stage 10
        OrderedSet([BitSet(1+4*i:4*(1+i)) for i in 0:1023]), # stage 11
        OrderedSet([BitSet(1+2*i:2*(1+i)) for i in 0:2047]), # stage 12
        OrderedSet([BitSet(i) for i in 1:4096]) # stage 13
    ]

    dim_to_subspace = [1:Df*2, Df*2+1:Df*4, Df*4+1:Df*6, Df*6+1:Df*8, Df*8+1:Df*10, Df*10+1:Df*12,
    Df*12+1:Df*14, Df*14+1:Df*16, Df*16+1:Df*18, Df*18+1:Df*20, Df*20+1:Df*22, Df*22+1:Df*24, Df*24+1:Df*26+Df*5+In*Df*2]

    custom_nscenarios = 4096
    custom_nstages =13

    scenariotree = ScenarioTree(; depth=13, nbranching=2)

    pb = Problem(
        scenarios,  # scenarios array
        build_fs_CsEV!,
        vec((1/4096).*ones(4096)),                  # scenario probabilities
        custom_nscenarios,
        custom_nstages,
        dim_to_subspace,
        scenariotree
