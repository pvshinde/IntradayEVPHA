...
@everywhere function build_fs_CsEV!(model::JuMP.Model, s::PriceScenariosp, id_scen::ScenarioId)
    ...
    objexpr = sum(sum(pA[t, d]*s.ID_ask_price[t] - pB[t, d]*s.ID_buy_price[t] for t in Tf) for d in Df)
                + sum(pD[d] * s.Up_price[d] - pU[d] * s.Dn_price[d] + pIm[d]*s.Up_price[d] - pIp[d]*s.Dn_price[d] +
...
#assuming three ID stages will correspond to 4 scenarios if nbranching=2, one BM stage
    scenario1 = PriceScenariosp([3, 4, 6], [2, 2, 1], [2, 4, 4, 3],[2, 4, 4, 3],[2, 4, 4, 3], [2 2; 1 0; 2 2],
    [2 2; 1 0; 2 2], [2 2; 3 4; 1 2],[4 3; 3 4; 4 2]) #IDpriceA, B, Upreg, Dnreg, SoCinit, Q, d0, DD
    scenario2 = PriceScenariosp([3, 5, 7], [2, 2, 1],[2, 4, 4, 3], [2, 4, 4, 3],[2, 4, 4, 3],[2 2; 1 0; 2 2],
    [2 2; 1 0; 2 2], [2 2; 3 4; 1 2],[2 2; 3 4; 1 2]) # SoC is for Nf that is 3, ID_A, ID_B, SOC_init, Q, d0, DD
    scenario3 = PriceScenariosp([4, 3, 6], [3, 2, 3],[2, 4, 4, 3], [2, 4, 4, 3],[2, 4, 4, 3],[2 2; 1 0; 2 2],
    [2 2; 1 0; 2 2], [2 2; 3 4; 1 2],[2 2; 3 4; 1 2])
    scenario4 = PriceScenariosp([4, 3, 8], [3, 2, 4],[2, 4, 4, 3],[2, 4, 4, 3],[2, 4, 4, 3],[2 2; 1 0; 2 2],
    [2 2; 1 0; 2 2], [2 2; 3 4; 1 2],[2 2; 3 4; 1 2])
