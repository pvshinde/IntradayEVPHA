...
@everywhere function build_fs_CsEV!(model::JuMP.Model, s::PriceScenariosp, id_scen::ScenarioId)
    ...
    SoC_init = [10, 12, 14, 15, 15, 20, 14, 17, 18, 12] # initial SOC for each EV when it arrives
    ...
    P_DA=[110, 120, 105, 101, 90, 85, 100, 114, 108, 90] # DA position of EV aggregator for each DP
...
scenarios = [PriceScenariosp([price_scenarios_1[i:i,:]; price_scenarios_2[i:i,:];price_scenarios_3[i:i,:]; price_scenarios_4[i:i,:];price_scenarios_5[i:i,:];
...
#IDpriceA, B, Upreg, Dnreg,Imprice, Ipprice, SoCinit, Q, d0, DD
    ...
    stageid_to_scenpart = [
        OrderedSet([BitSet(1:16)]),                      # Stage 1
        OrderedSet([BitSet(1+8*i:8*(1+i)) for i in 0:1]),           # Stage 2
        OrderedSet([BitSet(1+4*i:4*(1+i)) for i in 0:3]), #stage 3
        OrderedSet([BitSet(1+2*i:2*(1+i)) for i in 0:7]), #stage4
        OrderedSet([BitSet(i) for i in 1:16]) # stage 5
    ]

    dim_to_subspace = [1:Tf*Df, Tf*Df+1:Tf*Df*2, Tf*Df*2+1:Tf*Df*3, Tf*Df*3+1:Tf*Df*2+Df*5+In*Df*2]

    custom_nscenarios = 16
    custom_nstages =5

    scenariotree = ScenarioTree(; depth=5, nbranching=2)

    pb = Problem(
        scenarios,  # scenarios array
        build_fs_CsEV!,
        vec((1/16).*ones(16)),                  # scenario probabilities
        custom_nscenarios,
        custom_nstages,
        dim_to_subspace,
        scenariotree
